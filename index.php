...
<section class="relative min-h-screen flex items-center overflow-hidden">
    <div class="absolute inset-0">
        <img src="assets/img/bgindex.jpg" class="w-full h-full object-cover" alt="Background Desa Ngargosari">
    </div>
    <div class="absolute inset-0 bg-black/50"></div>

    <div class="relative z-10 max-w-7xl mx-auto px-6 text-white text-center md:text-left">
        <h1 class="text-3xl md:text-5xl font-bold leading-tight">
            <img src="assets/img/logo.png" class="w-28 mx-auto mb-4" alt="Logo Desa Ngargosari">
           <br> Selamat Datang di<br>Website Resmi Desa Ngargosari 
        </h1> 
        <p class="mt-4 max-w-xl mx-auto md:mx-0 text-white/90">
...
<!-- ================= MAP ================= -->
<section class="max-w-7xl mx-auto px-6 py-5 bg-white text-center mt-10 rounded-xl shadow">
    <h3 class="font-semibold py-4 text-lg">📍 Lokasi Desa Ngargosari</h3>
...
<!-- ================= PRODUK ================= -->
<section class="max-w-7xl mx-auto px-6 py-14 bg-white mt-10 rounded-xl shadow">
    <h2 class="text-2xl font-semibold text-green-800 mb-8 text-center">Produk Unggulan</h2>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        ...
        if ($qProduk && mysqli_num_rows($qProduk) > 0):
            while ($p = mysqli_fetch_assoc($qProduk)):
                $imgProduk = !empty($p['gambar']) ? "assets/img/produk/".htmlspecialchars($p['gambar']) : 'assets/img/no-image.png';
        ...
        <div class="bg-white rounded-xl shadow hover:shadow-xl transition overflow-hidden group">
            <img src="<?= $imgProduk ?>"
                 ...
                 onerror="this.src='assets/img/no-image.png'" alt="<?= htmlspecialchars($p['nama_produk']); ?>">
            ...
            <div class="p-4 text-center">
                ...
                <?php if (!empty($noWA)): ?>
                <a target="_blank" href="https://wa.me/<?= $noWA ?>"
                   class="inline-block mt-3 bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg text-sm">
                   💬 Pesan
                </a>
                <?php endif; ?>
            </div>
...
<!-- ================= BERITA TERBARU ================= -->
<section class="max-w-7xl mx-auto px-6 py-14 bg-white mt-10 rounded-xl shadow">
    <h2 class="text-2xl font-semibold text-green-800 mb-8 text-center">Berita Terbaru</h2>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        if ($qBerita && mysqli_num_rows($qBerita) > 0):
            while ($b = mysqli_fetch_assoc($qBerita)):
                $imgBerita = !empty($b['gambar']) ? "assets/img/berita/".htmlspecialchars($b['gambar']) : 'assets/img/no-image.png';
        ?>
        <div class="bg-white rounded-xl shadow hover:shadow-lg transition overflow-hidden">
            <img src="<?= $imgBerita ?>"
                 class="w-full h-40 object-cover"
                 onerror="this.src='assets/img/no-image.png'" alt="<?= htmlspecialchars($b['judul']); ?>">

            <div class="p-5">
                <h3 class="font-semibold text-lg"><?= htmlspecialchars($b['judul']); ?></h3>
                <p class="text-sm text-gray-600 mt-2"><?= substr(strip_tags($b['isi']), 0, 120); ?>...</p>

                <?php if(!empty($b['id'])): ?>
                <a href="berita-detail.php?id=<?= (int)$b['id']; ?>"
                   class="inline-block mt-3 text-green-700 font-semibold text-sm hover:underline">
                   Baca Selengkapnya →
                </a>
                <?php endif; ?>
            </div>
...
<!-- ================= GALERI ================= -->
<section class="max-w-7xl mx-auto px-6 py-14 bg-white mt-10 rounded-xl shadow">
    <h2 class="text-2xl font-semibold text-green-800 mb-8 text-center">
        Galeri Desa
    </h2>
