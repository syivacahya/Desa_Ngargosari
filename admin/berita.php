...
if (isset($_POST['simpan'])) {
    ...
    mysqli_query($koneksi, "INSERT INTO berita (judul, isi, gambar, tanggal) VALUES ('$judul', '$isi', '$gambar', '$tanggal')");
...
if (isset($_POST['update'])) {
    ...
    $update_gambar = "";
    if (!empty($_FILES['gambar']['name'])) {
        $old = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT gambar FROM berita WHERE id=$id"));
        if ($old && file_exists("../assets/img/berita/".$old['gambar'])) {
    ...
    mysqli_query($koneksi, "UPDATE berita SET judul='$judul', isi='$isi', tanggal='$tanggal' $update_gambar WHERE id=$id");
    header("Location: berita.php");
    exit;
}
?>
                ...
                <table class="w-full border border-gray-300 text-sm">
                    ...
                    <?php if (mysqli_num_rows($query) > 0): ?>
                        ...
                        <tr class="hover:bg-gray-50">
                            <td class="border px-4 py-2 text-center"><?= date('d-m-Y', strtotime($row['tanggal'])) ?></td>
                            <td class="border px-4 py-2 font-medium"><?= htmlspecialchars($row['judul']) ?></td>
                            <td class="border px-4 py-2 max-w-sm"><?= htmlspecialchars(mb_strimwidth($row['isi'], 0, 80, '…')) ?></td>
                            <td class="border px-4 py-2 text-center">
                                <?php if (!empty($row['gambar'])): ?>
                                    <img src="../assets/img/berita/<?= htmlspecialchars($row['gambar']) ?>" class="w-20 h-14 object-cover mx-auto rounded">
                                <?php else: ?>
                            ...
                            <td class="border px-4 py-2">
                                <div class="flex justify-center items-center gap-2">
                                    <button onclick='editBerita(<?= htmlspecialchars(json_encode($row), ENT_QUOTES) ?>)' 
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="5" class="border px-4 py-6 text-center text-gray-500">Belum ada data berita</td>
                        </tr>
                    <?php endif; ?>
                    </tbody>
                </table>
...
<div id="modalAdd" class="hidden fixed inset-0 flex items-center justify-center bg-black/50 z-50">
    <div class="bg-white p-6 rounded-xl w-96">
        <h2 class="font-bold text-lg mb-4">Tambah Berita</h2>
        <form method="post" enctype="multipart/form-data" class="space-y-4">
            ...
            <div>
                ...
                <textarea name="isi" rows="4" class="w-full border p-2 rounded" required></textarea>
            </div>
            <div>
...
<div id="modalEdit" class="hidden fixed inset-0 flex items-center justify-center bg-black/50 z-50">
    <div class="bg-white p-6 rounded-xl w-96">
        <h2 class="font-bold text-lg mb-4">Edit Berita</h2>
        <form method="post" enctype="multipart/form-data" class="space-y-4">
            <input type="hidden" name="id" id="editId">
            <div>
                <label>Judul</label>
                <input type="text" name="judul" id="editJudul" class="w-full border p-2 rounded" required>
            </div>
            <div>
                <label>Tanggal</label>
            ...
            <div>
                <label>Isi Berita</label>
                <textarea name="isi" id="editIsi" rows="4" class="w-full border p-2 rounded" required></textarea>
            </div>
            <div>
...
// Preview gambar saat pilih file baru di modal edit
